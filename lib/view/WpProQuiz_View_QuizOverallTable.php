...
class WpProQuiz_View_QuizOverallTable extends WP_List_Table {
	...
	function column_name($item) {
		$actions = array(
			'wpProQuiz_questions' => sprintf('<a href="?page=wpProQuiz&module=question&quiz_id=%s">'.__('Questions', 'wp-pro-quiz').'</a>', $item['ID']),
		);

		if(current_user_can('wpProQuiz_edit_quiz'))
			$actions['wpProQuiz_edit'] = sprintf('<a href="?page=wpProQuiz&action=addEdit&quizId=%s">'.__('Edit', 'wp-pro-quiz').'</a>', $item['ID']);

		if(current_user_can('wpProQuiz_delete_quiz'))
			$actions['wpProQuiz_delete'] = sprintf('<a style="color: red;" href="?page=wpProQuiz&action=delete&id=%s">'.__('Delete', 'wp-pro-quiz').'</a>', $item['ID']);

		$actions['wpProQuiz_preview'] = sprintf('<a href="?page=wpProQuiz&module=preview&id=%s">'.__('Preview', 'wp-pro-quiz').'</a>', $item['ID']);

		if(current_user_can('wpProQuiz_show_statistics'))
			$actions['wpProQuiz_statistics'] = sprintf('<a href="?page=wpProQuiz&module=statistics&id=%s">'.__('Statistics', 'wp-pro-quiz').'</a>', $item['ID']);

		if(current_user_can('wpProQuiz_toplist_edit'))
			$actions['wpProQuiz_leaderboard'] = sprintf('<a href="?page=wpProQuiz&module=toplist&id=%s">'.__('Leaderboard', 'wp-pro-quiz').'</a>', $item['ID']);

		return sprintf('<a class="row-title" href="?page=wpProQuiz&module=question&quiz_id=%1$s">%2$s</a> %3$s', $item['ID'], $item['name'], $this->row_actions($actions) );
	}
